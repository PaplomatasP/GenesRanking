...
#'
Statistical_Filter <-
    function(data, Pvalue_md, Labels, n_genes_to_keep) {
        options(scipen = 999)
        threshold <- 1
        ...
        if (Pvalue_md == "Waldtest" |
            ...
            Genes <-
                head(PvalueTreshold$Genes, n <- n_genes_to_keep) # Keep top n rows
...
KruskalWallisFun <- function(data, Labels, n_genes_to_keep) {
    ...
    kruskal_w <-
        apply(data[, -ncol(data)], 2, function(x) {
            kruskal.test(x ~ data$Labels)
        })

    # Extract p-values from the list
    p_values <- vapply(kruskal_w, function(x) {
      x$p.value
    }, FUN.VALUE = numeric(1))
    ...
    kruskal_w_rank <-
        kruskal_w_rank[order(kruskal_w_rank$FDR), ] # Sort by FDR column

    Genes <-
        head(kruskal_w_rank$Genes, n <- n_genes_to_keep) # Keep top n rows
...
mRMReFun <- function(data, Labels, n_genes_to_keep) {
    ...
    Genes <-
        apply(mRMRe::solutions(filter)[[1]], 2, function(x, y) {
            return(y[x])
        }, y = mRMRe::featureNames(filter))
...
QLFTfun <- function(data, Labels, n_genes_to_keep) {
    ...
    object_edgeR <-
        data.frame(apply(object_edgeR, 2, function(x) {
...
KendallFun <- function(data, Labels, n_genes_to_keep) {
    ...
    kendall_result <-
        apply(data[, -ncol(data)], 2, function(x) {
          Kendall::Kendall(x, data$Labels)
        })

    # Extract tau and p-values from the list
    kendall_tau <- vapply(kendall_result, function(x) {
      x$tau
    }, FUN.VALUE = numeric(1))
    
    p_values <- vapply(kendall_result, function(x) {
      x$sl
    }, FUN.VALUE = numeric(1))
    ...
    kendall_rank <-
        kendall_rank[order(kendall_rank$FDR), ] # Sort by FDR column

    Genes <-
        head(kendall_rank$Genes, n = n_genes_to_keep) # Keep top n rows
    data <- data[, -ncol(data)]
    data <- as.data.frame(t(data))
    neudata <- data[rownames(data) %in% Genes, ]
...
CorrelationFun <- function(Data, Labels, n_genes_to_keep) {
    ...
    selected_features <-
        names(sorted_features)[seq_len(n_genes_to_keep)]
    # Filter the data based on the selected genes
...
ig_fun <- function(Data, Labels, n_genes_to_keep) {
    ...
    sorted_features <-
        ig_scores[order(ig_scores$attr_importance, decreasing = TRUE), ]
    # Select top n_genes_to_keep features
    selected_features <-
        rownames(sorted_features)[seq_len(n_genes_to_keep)]
