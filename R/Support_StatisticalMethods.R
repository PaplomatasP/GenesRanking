...
#'
data_fitting <- function(data, group) {
    # normal.Method
    SingleCell <-
        SingleCellExperiment::SingleCellExperiment(assays = list(counts = data, normcounts = data))
...
#' PvalueCalc function
#'
#' This function calculates p-values and FDRs for differential expression
#' analysis using different methods.
#'
#' @param data a SummarizedExperiment object containing gene expression data
#' @param Pvaluemethod a character string specifying the method to be used for
#'   calculating p-values and FDRs. Possible values are "Seurat_method",
#'   "BPSC_metchod", "DESeq2_method", and "MAST_method".
#' @param Labels a vector of group labels for the samples in the data object
#' @param Test a character string specifying the test to be used in the
#'   DESeq2 method. Default is "LRT".
#' @param dmethod a character string specifying the method to be used in the
#'   Seurat method. Default is "bimod".
#' @param DESeq2.test a character string specifying the test to be used in the
#'   DESeq2 method. Default is "LRT".
#' @param method_MAST a character string specifying the method to be used in
#'   the MAST method. Default is "bayesglm".
#' @param MAST.parallel a logical value indicating whether parallel computing
#'   should be used in the MAST method. Default is TRUE.
#' @return A list with gene_names, pvalue, and FDR values
...
PvalueCalc <- function(data,
    ...
    if (Pvaluemethod == "WilcoxonTest") {
        ...
        Seurat.input <-
            Seurat::CreateSeuratObject(counts = object_Seurat, meta.data = meta.data)
        Seurat.input <- Seurat::NormalizeData(object = Seurat.input)
        res <- Seurat::FindMarkers(
            object = Seurat.input,
            ident.1 = levels(as.factor(tmp))[1],
            ident.2 = levels(as.factor(tmp))[2],
            group.by = "groups",
            logfc.threshold = -Inf,
            test.use = dmethod,
            only.pos = FALSE,
        ...
        object_DESeq2 <-
            as.matrix(SummarizedExperiment::assay(data, "counts"))
        colnames(object_DESeq2) <- NULL
        object_DESeq2 <-
            DESeq2::DESeqDataSetFromMatrix(
                countData = round(object_DESeq2 + 1),
        ...
        if (DESeq2.test == "LRT") {
          ...
          object_DESeq2 <- DESeq2::estimateSizeFactors(object_DESeq2)
          
          # Estimate dispersions
          object_DESeq2 <- DESeq2::estimateDispersionsGeneEst(object_DESeq2)
          
          # Use gene-wise estimates as final estimates
          DESeq2::dispersions(object_DESeq2) <- S4Vectors::mcols(object_DESeq2)$dispGeneEst
          
          object_DESeq2 <- DESeq2::nbinomLRT(object_DESeq2, reduced = ~1)
          
          }
        if (DESeq2.test == "Wald") {
            object_DESeq2 <- DESeq2::DESeq(
                object_DESeq2,
                test = DESeq2.test,
                parallel = TRUE,
                betaPrior = TRUE,
        ...
        zlmdata <- MAST::zlm(
            ~grp, sca, method <- method_MAST,
            parallel <- MAST.parallel
        )
        mast <- MAST::lrTest(zlmdata, "grp")
        FDR <- p.adjust(mast[, "hurdle", "Pr(>Chisq)"], method <- "BH")
        result_MAST <-
            list(
                gene_names <- names(mast[, "hurdle", "Pr(>Chisq)"]),
