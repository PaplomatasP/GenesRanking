...
#'
#' This function performs a statistical filtering method on gene expression data. It filters the genes based on their statistical significance calculated by various methods.
#'
...
#' @param Pvalue_md A character string indicating the statistical method to use for filtering. Can be one of "Waldtest", "BPglm", "Wilcoxon-Test" or "LRT".
...
#' \item{Statistical_Analysis}{A data.frame object containing the p-values and FDRs for all genes, as calculated by the selected statistical method.}
#'
#' @examples
#'
#' data(ExampleDataset)
#' Statistical_Filter(ExampleDataset[1:10,],Pvalue_md="Waldtest",Labels=Labels,n_genes_to_keep = 100)
#'
  ...
  if (Pvalue_md == "Waldtest" |
    ...
    for (i in seq(1, nrow(PvalueData))) {
      if (PvalueData[, 3][i] <= threshold) {
        ...
        PvalueTreshold = rbind(PvalueTreshold, list(rownames(PvalueData)[i], PvalueData[,2][i]) )
      }
    }
