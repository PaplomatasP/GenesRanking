...
#'
#' This function applies non-tree-based machine learning-based feature selection to single-cell RNA sequencing (scRNA-seq) data. The function takes in a data matrix, a vector of cell labels, a machine learning method, and additional parameters to configure the machine learning method. The resulting filtered data , important features and the ML analysis are returned as a list.
#'
...
#' @param MLmethod A character string specifying the non-tree machine learning method to be used. The valid options are "knn", "svmRadial", "glmnet", "lda".
...
#' \code{Important_Features} is a vector of important genes selected by the non-tree machine learning-based feature selection method.
#' \code{ML_Analysis} is a data frame containing the importance scores for all features used in the non-tree machine learning analysis.
#'
...
#'
#' data(ExampleDataset)
#' Non_Tree_Based_ML(ExampleDataset[1:10,],Labels=Labels,"knn",n_genes_to_keep = 100)
#' 
#' @export
Non_Tree_Based_ML = function(data,
                            Labels,
                            MLmethod,
