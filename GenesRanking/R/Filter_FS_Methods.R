...
Filter_FS_Methods <- function(data, Labels, n_genes_to_keep, method,
  ...
  if (method == "WaldTest") {
    result <- Statistical_Filter(data, Pvalue_md="Waldtest", Labels=Labels, n_genes_to_keep=n_genes_to_keep)
  } else if (method == "BetaPoissonGLM") {
    result <- Statistical_Filter(data, Pvalue_md="BPglm", Labels=Labels, n_genes_to_keep=n_genes_to_keep)
  } else if (method == "WilcoxonTest") {
    result <- Statistical_Filter(data, Pvalue_md="WilcoxonTest", Labels=Labels, n_genes_to_keep=n_genes_to_keep)
  } else if (method == "LikelihoodRatioTest") {
    result <- Statistical_Filter(data, Pvalue_md="LRT", Labels=Labels, n_genes_to_keep=n_genes_to_keep)
  } else if (method == "KruskalWallis") {
    result <- KruskalWallisFun(data, Labels=Labels, n_genes_to_keep=n_genes_to_keep)
  } else if (method == "mRMRe") {
    result <- mRMReFun(data, Labels=Labels, n_genes_to_keep=n_genes_to_keep)
  } else if (method == "QuasiLikelihoodFTest") {
    result <- QLFTfun(data, Labels=Labels, n_genes_to_keep=n_genes_to_keep)
  } else if (method == "KendallCorrelation") {
    result <- KendallFun(data, Labels=Labels, n_genes_to_keep=n_genes_to_keep)
  } else if (method == "SpearmanCorrelation") {
    result <- CorrelationFun(data, Labels=Labels, n_genes_to_keep=n_genes_to_keep)
  } else if (method == "InformationGain") {
    result <- ig_fun(data, Labels=Labels, n_genes_to_keep=n_genes_to_keep)
  } else {
