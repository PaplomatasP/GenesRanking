...
#'   "knn", "svmLinear", "lda", "rf", "rpart", "C5.0", "gbm", and "xgbTree".
...
ML_filter <- function(data,
  ...
  #'
  #'
  CatBoost_feature_selection <- function(data, Labels, n_genes_to_keep) {
    ...
    train_index <- caret::createDataPartition(Labels, p = 0.8, list = FALSE)
    ...
    train_pool <- catboost::catboost.load_pool(data = train_data, label = train_labels)
    test_pool <- catboost::catboost.load_pool(data = test_data, label = test_labels)
    ...
    feature_importance <-as.data.frame(catboost::catboost.get_feature_importance(model, train_pool,
                                                                        type = "FeatureImportance") )

    feature_importance <- feature_importance[order(-feature_importance[, 1]), , drop <- FALSE]
    # Get the top n_genes_to_keep genes
    top_genes_indices <- unlist(head(rownames(feature_importance), n_genes_to_keep) )

    # Filter the data to keep only the top genes
    filtered_data <- data[, top_genes_indices]

    return(list(FilteredData =  as.data.frame(t(filtered_data)),
  ...
  ShapleyValuesFun <- function(data, Labels, n_genes_to_keep) {
    ...
    pfun <- function(object, newdata) {  #extracts prediction from ranger
    ...
    model <- ranger::ranger(Labels ~ ., data = data, probability = TRUE, num.trees = 100)
    ...
    for(i in 1:length(class_names)){
      ...
      tmp_res = fastshap::explain(model,X = data, newdata = data[c,],pred_wrapper = pfun)
    ...
    rank <- rank[order(-rank$Shapley), ] # Sort descending (it is already sorted!)
