...
#'
#' This function filters genes in scRNA-seq data according to the specified
#' method. The user can choose between two filtering methods:
#' "High_Variable_Genes" and "Remove_Low_Variance". If "High_Variable_Genes" is
#' selected, the function uses the FindVariableFeatures function of Seurat to
#' identify the top-ranked variable genes. If "Remove_Low_Variance" is selected,
#' the function removes genes with low variance using the nearZeroVar function
#' of caret.
#'
#' @param data A numeric matrix of gene expression values with rows representing
#'   genes and columns representing cells.
#' @param filter_method A character string specifying the filtering method to be
#'   used. The valid options are "High_Variable_Genes" and "Remove_Low_Variance".
#' @param n_features An integer value that sets the number of top-ranked variable
#'   genes to keep. Only used when filter_method is "High_Variable_Genes".
#' @param unique_cut A numeric value that sets the threshold for low variance
#'   genes. Only used when filter_method is "Remove_Low_Variance".
#'
#' @return A filtered data frame with rows representing genes and columns
#'   representing cells.
#'
#' @examples
#'
...
normalize_data <- function(ExpressionData, normal_method) {
  if (normal_method == "TMM") {
    ...
    SF <- function(cnts) {
      norm_factor <- stats::median(
        (cnts / geomeans)[is.finite(geomeans) & geomeans > 0])
    }
    ...
    Matrix_data <- SingleCellExperiment::SingleCellExperiment(
      assays = list(counts = ExpressionData))
    ...
    normalized_data <- as.matrix(
      Seurat::LogNormalize(ExpressionData, scale.factor <- scale_factor))
  }
