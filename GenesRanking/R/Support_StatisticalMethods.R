...
#'
PvalueCalc <- function(data, Pvaluemethod, Labels, Test = "LRT",
                       dmethod = "bimod", DESeq2.test = "LRT",
                       method_MAST = "bayesglm", MAST.parallel = TRUE) {
  if (Pvaluemethod == "WilcoxonTest") {
    ...
    res <- Seurat::FindMarkers(object = Seurat.input,
    ...
    result_BPSC <- list(gene_names = names(resbp$PVAL),
                        pvalue = resbp$PVAL,
                        FDR = FDR)
    return(result_BPSC)
    ...
    sca <- MAST::FromMatrix(
      ...
      cData <- data.frame(wellKey <- names(grp),
                         grp = grp))
    zlmdata <- MAST::zlm(~ grp, sca, method <- method_MAST,
